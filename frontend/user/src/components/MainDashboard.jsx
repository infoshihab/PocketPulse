...
export default function MainDashboard() {
  ...
  const cards = useMemo(
    () => [
      ...
      { key: "rent", label: "Rent", icon: <FaHome />, color: "#2bcbba" },
      {
        key: "utility",
        label: "Utility",
        icon: <FaLightbulb />,
    ...
    return (
      <div className="p-6">
        <div className="bg-white rounded-xl shadow p-6 text-center">
          <h2 className="text-xl font-semibold mb-2">Please log in first</h2>
  ...
  return (
    <div className="p-3 space-y-4">
      <div className="text-left">
        ...
        <p className="text-gray-500">Showing totals for this month</p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
        {cards.map((c, i) => (
          <article
            key={i}
            aria-label={c.label}
            className="relative flex flex-col justify-between p-6 h-44 rounded-2xl shadow-md hover:shadow-xl transition-all duration-300"
            style={{
              background: `linear-gradient(135deg, ${c.color}, ${c.color}cc)`,
            }}
          >
            <div>
              <h2 className="text-white text-lg font-semibold">{c.label}</h2>
              <p className="text-white text-2xl font-bold">
                {formatShort(totalsByKey[c.key])}
              </p>
            </div>
            <div className="absolute bottom-4 right-4 bg-white/20 backdrop-blur-md p-3 rounded-full text-white shadow-md text-xl">
              {c.icon}
            </div>
          </article>
        ))}
          ...
          <thead>
            <tr className="bg-gray-900 text-white text-sm">
              {["Summary", "Amount", "Date", "Category", "Actions"].map(
                (th, i) => (
                  <th key={i} className="py-3 px-4 text-center font-medium">
                    {th}
                  </th>
                )
              )}
            ...
            {thisMonthItems?.length ? (
              [...thisMonthItems]
                ...
                .map((item, idx) => (
                  ...
                  >
                    <td className="py-3 px-4 border">{item.summary}</td>
                    ...
                    <td className="py-3 px-4 italic border text-center">
                      {item.category}
                    </td>
